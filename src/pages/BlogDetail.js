...
import React from 'react';
import { useParams, Link } from 'react-router-dom';
import { Clock, ArrowLeft, Share2, Calendar, User } from 'lucide-react';
...
const BlogDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      {/* Hero Section */}
      <div className="relative bg-gradient-to-r from-blue-900 via-purple-900 to-indigo-900 text-white py-32 px-6">
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
        <div className="relative max-w-4xl mx-auto">
          {/* Navigation */}
          <div className="flex items-center space-x-2 mb-8 text-gray-300 text-sm">
            <Link to="/" className="flex items-center hover:text-white transition-colors duration-300">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        {/* Share Button */}
        <div className="fixed left-8 top-1/2 transform -translate-y-1/2 hidden lg:flex flex-col items-center gap-4">
          <button className="p-3 rounded-full bg-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110">
        ...
        <article className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
          <div className="prose prose-lg max-w-none">
            <p className="text-lg leading-relaxed text-gray-700 whitespace-pre-line first-letter:text-7xl first-letter:font-bold first-letter:float-left first-letter:mr-3 first-letter:mt-2 first-letter:font-serif">
              {post.content}
            </p>
          </div>
          ...
          <div className="mt-12 pt-8 border-t border-gray-200">
            ...
            <div className="flex flex-wrap gap-2">
              {post.tags.map((tag, index) => (
                <span key={index} className="px-4 py-2 rounded-full bg-gray-100 text-gray-700 text-sm hover:bg-gray-200 transition-colors duration-300 cursor-pointer">
                  #{tag}
                </span>
              ))}
            </div>
